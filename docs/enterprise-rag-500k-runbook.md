...
Apply and review V1 and V2 with a privileged `psql` session. Do not use the Supabase table editor for a multi-million-row import.
...
The worker writes only to a new staging corpus and records every completed document in SQLite WAL mode. Re-run with `--resume` after a process/network failure. Inspect count, disk, WAL, tokens, cost and p95 latency before moving to 5K and 50K.
